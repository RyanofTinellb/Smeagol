...
def evolve(text, rewrites, replacements):
...
with open('c:/users/ryan/documents/tinellbianlanguages'
    ...
    wordlist = [entry['t'] for entry in json.load(f)
        if entry['l'] == 'High Lulani' and 'verb' in entry['p']]
    wordlist = [evolve(x, rewrites, replacements) for y in [
    ...
    for x in y]
    # wordlist = unique([evolve(entry['t'], rewrites, replacements) for entry in json.load(f) if entry['l'] == 'High Lulani'])
with open('c:/users/ryan/documents/tinellbianlanguages'
