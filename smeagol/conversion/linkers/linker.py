...
from smeagol.conversion.linkers.internal_dictionary import InternalDictionary
...
class ExternalDictionary:
    ...
    def _unlink(self, regex):
        ...
        if lang == utils.urlform(self.language):
            return f'<{tag}>{link}</tag>'.format(tag, link)
        return f'<{tag}>{tr.encode(lang)}:{link}</{tag}>'.format(tag, tr.encode(lang), link)

    def wordlist_setup(self):
        wordlist = fs.load_yaml(self.wordlist_file)
        self.wordlist = [word['t'] for word in wordlist]

    def refresh(self, _text=None):
        self.wordlist_setup()
